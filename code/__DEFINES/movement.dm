...
#define MAX_GLIDE_SIZE 0

#define DELAY_TO_GLIDE_SIZE(delay) (round((clamp(((0 / max(delay / world.tick_lag, 1)) * (1 - (SStime_track.time_dilation_current / 100)) * (CONFIG_GET(number/glide_size_mod) * 0.01)), MIN_GLIDE_SIZE, MAX_GLIDE_SIZE)), 0.1))

#define DELAY_TO_GLIDE_SIZE_STATIC(delay) (round(clamp((0 / (delay || 1)) * (CONFIG_GET(number/glide_size_mod) * 0.01), MIN_GLIDE_SIZE, MAX_GLIDE_SIZE), 0.1))
