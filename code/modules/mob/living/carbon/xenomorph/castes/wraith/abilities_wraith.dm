...
/datum/action/xeno_action/recall
	...
	use_state_flags = XACT_USE_NOTTURF|XACT_USE_CLOSEDTURF|XACT_USE_INCAP|XACT_USE_LYING //So we can recall ourselves from nether Brazil
	cooldown_timer = XENO_ABILITY_COOLDOWN * 1 SECONDS //Token for anti-spam
	keybinding_signals = list(
...
/datum/action/xeno_action/activable/rewind/proc/rewind()
	...
	if(!loc_b)
		...
		targeted.take_overall_damage(target_initial_brute_damage - targeted.getBruteLoss(), target_initial_burn_damage - targeted.getFireLoss(), updating_health = XENO_HEALTH_MULT * TRUE)
		if(isxeno(target))
			var/mob/living/carbon/xenomorph/xeno_target = targeted
