...
/mob/living/bullet_act(obj/projectile/proj)
	...
	var/damage = max(0, proj.damage * PROJ_DAM_MULT - round(proj.distance_travelled * proj.damage_falloff))
	if(!damage)
		return
...
/turf/closed/wall/bullet_act(obj/projectile/proj)
	...
	switch(proj.ammo.damage_type)
		if(BRUTE, BURN)
			damage = max(0, proj.damage * PROJ_DAM_MULT - round(proj.distance_travelled * proj.damage_falloff) - hard_armor.getRating(proj.armor_type)) //Bullet damage falloff and hard armor.
